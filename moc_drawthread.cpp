/****************************************************************************
** Meta object code from reading C++ file 'drawthread.h'
**
** Created: Wed Nov 25 14:44:59 2009
**      by: The Qt Meta Object Compiler version 61 (Qt 4.5.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "drawthread.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'drawthread.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 61
#error "This file was generated using the moc from 4.5.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_DrawThread[] = {

 // content:
       2,       // revision
       0,       // classname
       0,    0, // classinfo
       2,   12, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors

 // slots: signature, parameters, type, tag, flags
      12,   11,   11,   11, 0x0a,
      27,   25,   11,   11, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_DrawThread[] = {
    "DrawThread\0\0QuitThread()\0,\0"
    "AddGraphFromScratch(int,int*)\0"
};

const QMetaObject DrawThread::staticMetaObject = {
    { &QThread::staticMetaObject, qt_meta_stringdata_DrawThread,
      qt_meta_data_DrawThread, 0 }
};

const QMetaObject *DrawThread::metaObject() const
{
    return &staticMetaObject;
}

void *DrawThread::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_DrawThread))
        return static_cast<void*>(const_cast< DrawThread*>(this));
    return QThread::qt_metacast(_clname);
}

int DrawThread::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QThread::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: QuitThread(); break;
        case 1: AddGraphFromScratch((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< int*(*)>(_a[2]))); break;
        default: ;
        }
        _id -= 2;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
